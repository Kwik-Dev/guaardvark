...
def test_casting_director_routes_to_existing_lora():
    canned = '''{"actions": [
        {"subject_name": "Alex", "action": "use_existing_lora", "existing_lora_id": 42}
    ]}'''
    agent = CastingDirector(llm=lambda **kw: canned)
    inv = agent.invoke({
        "subjects": [{"name": "Alex", "kind": "character"}],
        "library": [{"id": 42, "name": "Alex", "kind": "character"}],
    })
...
def test_casting_director_routes_to_train_from_uploads():
    canned = '''{"actions": [
        {"subject_name": "Alex", "action": "train_from_uploads"}
    ]}'''
    agent = CastingDirector(llm=lambda **kw: canned)
    inv = agent.invoke({
        "subjects": [{"name": "Alex", "kind": "character"}],
