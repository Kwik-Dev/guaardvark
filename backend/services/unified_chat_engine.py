...
class UnifiedChatEngine:
    ...
    def _run_chat(self, session_id: str, message: str, options: Dict[str, Any],
        ...
        # live chat path is byte-identical to the XML+regex path until the
        # on-path is validated against a live tool-capable model.
        #
    ...
    def _call_llm_streaming(self, messages: List[Dict[str, str]], emit_fn: Callable,
        ...
        try:
            ...
            if _use_mistral:
                # Cloud provider stays on the simple XML tool-call path (native
                # cloud tool-calling is deferred — see #38 design). No keep_alive
                # (not an Ollama-managed local model).
                ...
                # Local Ollama path. Native tool-calling path (feature-flagged via
                # GUAARDVARK_NATIVE_TOOLCALLS, gated additionally on model 'tools'
                # capability — see _run_chat). When active we pass Ollama's native
                # tools=[...] schema; the model returns structured tool_calls in
                # message.tool_calls rather than inline XML.
