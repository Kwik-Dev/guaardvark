...
def _resolve_cast_from_prompt(text: str) -> list[int]:
    ...
    def _match_rows(rows) -> list[int]:
        """Phrase/name matching over Subject rows or Cast Library subject dicts."""
        found: list[int] = []
        seen: set[int] = set()
    ...
    try:
        ...
        if in_mcp_process():
            ...
            return _match_rows(
                [s for s in subjects if s.get("kind") == "character" and s.get("lora_path")]
            )
